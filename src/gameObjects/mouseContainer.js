...
export class MouseContainer extends Phaser.GameObjects.Container{
    ...
    constructor(scene, x, y, size){
        ...
        'Con meo, con cho', 
        { color: 'black', fontSize: 13, align: 'center', wordWrap: { width: size } }).setOrigin(0.5);
        ...
        this.sprite.on('pointerdown', (event) => {
            const {currentAnim} = this.sprite.anims;
            if(currentAnim && ['mouse-up'].indexOf(currentAnim.key) !== -1){
                ...
                if(this.isCorrect){
                    ...
                    this.scene.sound.play('angry', { rate: 1.2 });
        ...
        this.sprite.on('animationcomplete', (event)=>{
            if(event.key === 'mouse-up'){
                this.scene.time.addEvent({
                    ...
                    loop: false,
                    delay: Phaser.Math.FloatBetween(5, 8) * 1000
                });
            }
    ...
    mouseUp() {
        ...
        this.scene.time.addEvent({
            ...
            loop: false,
            delay: Phaser.Math.FloatBetween(8, 10) * 1000
        });
    }
}